...
import { BadgeCheck, Receipt, RotateCcw, User } from 'lucide-react';
...
export function SalesList({ rows, view }: { rows: OrderRow[]; view: AdminView }) {
  ...
  const items: ResourceItem[] = rows.map((row) => ({
    ...
    badges: [
      // Title-cased: `paid`/`pending`/`refunded` are column values, not labels.
      { label: row.status.charAt(0).toUpperCase() + row.status.slice(1), tone: STATUS_TONE[row.status] ?? 'rose' },
      { label: row.gateway, tone: 'slate' as const },
      ...(row.creditsGranted ? [{ label: 'credited', tone: 'slate' as const }] : []),
    ],
    // The customer's name is already in the subtitle beside the pack name.
    meta: [{ label: 'Amount', value: row.amount }],
    actions: [
  ...
  return <ResourceView module="sales" view={view} items={items} empty={{ icon: Receipt, title: 'No orders yet', description: 'Every credit pack purchase appears here with its payment status and gateway.' }} columns={3} />;
}
