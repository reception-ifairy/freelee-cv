        ...
        <nav className="h-fit space-y-1 rounded-2xl border border-slate-200/80 bg-white p-2 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
          {docs.map((d) => (
            <Link
              ...
              className={cn(
                'flex items-baseline gap-2 rounded-lg px-3 py-2 text-sm font-medium transition',
                d.file === active.file
                  ? 'bg-brand-50 text-brand-700 dark:bg-brand-500/15 dark:text-brand-300'
                  : 'text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800',
              )}
            >
              {/* The docs are numbered files (00-overview, 33-block-builder)
                  and the list is ordered by that number, but the number was
                  stripped along with the filename — so a deliberately ordered
                  reference read as an arbitrary alphabetical pile. */}
              <span className="shrink-0 font-mono text-[10px] text-slate-500">{d.file.slice(0, 2)}</span>
